...
def randCent(dataSet, K):
    ...
    for k in range(1,K):
        ...
        Dx is an array of minimum-distances of all points from any of the elected_centers
...
def kMeansParallel(dataSet, k, distMeas=distEclud, createCent=randCent):
    m = shape(dataSet)[0]
    ...
    dataSet = mat(dataSet)
    #centroids = centroids.resize((20,1))
    print('centroids are:',centroids)
    img = Image.open('config/parallel_kmean.png')
    img.show()
    return end_time
...
def kMeans(dataSet, k, distMeas=distEclud, createCent=randCent):
    ...
    #centroids = centroids.resize((20,1))
    print('centroids are:',centroids)
    #img = Image.open('parallel_kmean.png')
    #img.show()
    clusterChanged = True
